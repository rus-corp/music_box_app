...
export default function SettingsScreen() {
  const handleSendTrackLogs = async () => {
    const handleTrackLogs = await sendTrackLogs()
    if (handleTrackLogs.status === 201) {
      Alert.alert('Логи Треков сохранены', 'Нажмите ок', [
        { text: 'OK' }
      ])
  ...
  const handleSendAppLogs = async () => {
    const handleAppLogs = await sendAppLogs()
    if (handleAppLogs.status === 201) {
      Alert.alert('Логи приложения сохранены', 'Нажмите ок', [
        { text: 'OK'}
      ])
    ...
    <View style={styles.mainContainer}>
      ...
      >
        <View style={styles.blockContent}>
          <Button title='отправить логи приложения' onPress={handleSendAppLogs}/>
          <Button title='отправить логи треков' onPress={handleSendTrackLogs}/>
